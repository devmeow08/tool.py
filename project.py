...
class johnTool_SMS_SYSTEM:
    ...
    def display_submenu(self):
        ...
        print(f'{W}{GR}             :X- M$$$$  {R}  *{GR}  \'#T#$~!8$WUXU~')
        print(f'{W}{GR}          :%\'  ~%$Mm:         ~!~ ?$$$$$')
        print(f'{W}{GR}          :! .-   ~T$8xx.  .xWW- ~””##*\'\'')
        print(f'{W}{GR}  .....   -~~:<  !    ~?T$@@W@*?$ {R} * {GR} /’')
        ...
        banner = [
            ...
            "╚═╗║║║╚═╗   ║║║║╣ ║║║║ ║",
            "╚═╝╩ ╩╚═╝   ╩ ╩╚═╝╝╚╝╚═╝",
            print(f"\033[36mᴊᴏʜɴ ᴍɪᴄʜᴀᴇʟ ᴄᴜᴛᴇ\033[0m")
        ]
        for line in banner:
            print(line)
...
def get_device_info():
    ...
    unique_str = f"{info['device_name']}|{info['brand']}|{info['model']}|{info['build_number']}"
    secure_id = hashlib.md5(unique_str.encode(errors='ignore')).hexdigest()
    return info, secure_id
...
if __name__ == "__main__":
    os.system('clear' if os.name == 'posix' else 'cls')
    dev_info, secure_id = get_device_info()

    print(f"\033[36m┌───────────────────────────────────────────────────────┐\033[0m")
    print(f"\033[35mDEVICE NAME:\033[32m {dev_info['device_name']:<40} \033[0m")
    print(f"\033[35mBRAND:      \033[32m {dev_info['brand']:<40} \033[0m")
    print(f"\033[35mMODEL:      \033[32m {dev_info['model']:<40} \033[0m")
    print(f"\033[35mCHIPSET:    \033[32m {dev_info['chipset']:<40} \033[0m")
    print(f"\033[35mANDROID VER:\033[32m {dev_info['android_version']:<40} \033[0m")
    print(f"\033[35mBUILD:      \033[32m {dev_info['build_number']:<40} \033[0m")
    print(f"\033[36m└───────────────────────────────────────────────────────┘\033[0m")
    print()
    print(f"\033[36m┌───────────────────────────────────────────────────────┐\033[0m")
    print(f"\033[36mSECURE DEVICE ID: {secure_id:<45} \033[0m")
    print(f"\033[36m└───────────────────────────────────────────────────────┘\033[0m")
    print()
    user_input_key = input(f"\033[37mEnter your key: \033[32m").strip()

    if user_input_key not in VALID_KEYS:
        print(f"\033[31m❌ INVALID KEY! ACCESS DENIED.\033[0m")
        exit()

    current_id = secure_id
    records = load_records()
    max_dev = VALID_KEYS[user_input_key]

    if user_input_key in records:
        if current_id in records[user_input_key]:
            print(f"\033[32m✅ KEY ACCEPTED! WELCOME.\033[0m")
            print(f"\033[36m🚀 STARTING SYSTEM...\033[0m\n")
            app = johnTool_SMS_SYSTEM()
            app.start()
        else:
            print(f"\033[31m❌ KEY ALREADY USED ON ANOTHER DEVICE!\033[0m")
            exit()
    else:
        if len(records.get(user_input_key, [])) >= max_dev:
            print(f"\033[31m⚠️ KEY LIMIT REACHED!\033[0m")
            exit()
        save_record(user_input_key, current_id)
        print(f"\033[32m✅ DEVICE REGISTERED SUCCESSFULLY!\033[0m")
        print(f"\033[36m🚀 STARTING SYSTEM...\033[0m\n")
        app = johnTool_SMS_SYSTEM()
        app.start()
